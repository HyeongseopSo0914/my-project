...
@WebServlet("/DeletereportboardServlet")
public class DeletereportboardServlet extends HttpServlet {
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (idParam == null || idParam.trim().isEmpty()) {
            ...
            response.getWriter().println("location.href='" + request.getContextPath() + "/reportboard';");
            response.getWriter().println("</script>");
            return;
        }
        ...
        try {
            ...
            try (Connection conn = DBUtil.getConnection();
                 PreparedStatement pstmt = conn.prepareStatement("DELETE FROM report_board WHERE report_id = ?")) {
